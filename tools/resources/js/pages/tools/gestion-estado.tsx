...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Gestión Estado', href: '/tools/gestion-estado' },
];
...
export default function GestionEstado({ estados, filters, stats }: PageProps) {
    const { flash } = usePage<SharedData>().props;
    const acciones = usePermisosVista('gestion-estado');
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            router.get('/tools/gestion-estado', search ? { search } : {}, {
                preserveState: true,
                preserveScroll: true,
                replace: true,
            });
    ...
    const submit = (e: FormEvent) => {
        ...
        if (editingId) {
            form.put(`/tools/gestion-estado/${editingId}`, options);
        } else {
            form.post('/tools/gestion-estado', options);
        }
    };

    const toggleEstado = (row: EstadoRow) => {
        router.put(
            `/tools/gestion-estado/${row.id}`,
            {
                Nombre: row.Nombre,
                Estado: !row.Estado,
                Observacion: row.Observacion ?? '',
    ...
    const confirmDelete = () => {
        ...
        router.delete(`/tools/gestion-estado/${deleteTarget.id}`, {
            preserveScroll: true,
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                                Administra los estados de radicado del sistema.
                            </p>
                        </div>
                    </div>
                    {acciones.crear && (
                    ...
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {isEditing
                                ...
                                : 'Registra un nuevo estado de radicado.'}
                        </DialogDescription>
                    </DialogHeader>
