...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Gestión de Motivo', href: '/tools/gestion-motivo' },
];
...
const emptyForm: MotivoForm = {
    Nombre: '',
    Estado: true,
    Observacion: '',
};

export default function GestionMotivo({ motivos, filters, stats }: PageProps) {
    const { flash } = usePage<SharedData>().props;
    const acciones = usePermisosVista('gestion-motivo');
    const [search, setSearch] = useState(filters.search ?? '');
    const [formOpen, setFormOpen] = useState(false);
    const [editingId, setEditingId] = useState<number | null>(null);
    const [viewRow, setViewRow] = useState<MotivoRow | null>(null);
    const [deleteTarget, setDeleteTarget] = useState<MotivoRow | null>(null);
    ...
    const form = useForm<MotivoForm>({ ...emptyForm });
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            router.get('/tools/gestion-motivo', search ? { search } : {}, {
                preserveState: true,
                preserveScroll: true,
                replace: true,
            });
    ...
    const submit = (e: FormEvent) => {
        ...
        if (editingId) {
            form.put(`/tools/gestion-motivo/${editingId}`, options);
        } else {
            form.post('/tools/gestion-motivo', options);
        }
    };

    const toggleEstado = (row: MotivoRow) => {
        router.put(
            `/tools/gestion-motivo/${row.id}`,
            {
                Nombre: row.Nombre,
                Estado: !row.Estado,
                Observacion: row.Observacion ?? '',
    ...
    const confirmDelete = () => {
        ...
        router.delete(`/tools/gestion-motivo/${deleteTarget.id}`, {
            preserveScroll: true,
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                                Administra los motivos del sistema.
                            </p>
                        </div>
                    </div>
                    {acciones.crear && (
                ...
                <div className="flex flex-1 flex-col overflow-hidden rounded-xl border bg-card shadow-sm">
                    ...
                    <div className="flex flex-col items-center justify-between gap-3 border-t p-4 sm:flex-row">
                        <span className="text-sm text-muted-foreground">
                            {motivos.total > 0
                                ? `Mostrando ${motivos.from}–${motivos.to} de ${motivos.total}`
                                : 'Sin registros'}
                        </span>
                        <div className="flex items-center gap-2">
                            <Button
                                ...
                                size="sm"
                                className="gap-1"
                                disabled={motivos.current_page <= 1}
                                onClick={() => goToPage(motivos.current_page - 1)}
                            >
                                <ChevronLeft className="size-4" />
                                Anterior
                            </Button>
                            <span className="px-1 text-sm text-muted-foreground">
                                Página {motivos.current_page} de{' '}
                                {motivos.last_page}
                            </span>
                            <Button
                                variant="outline"
                                ...
                                className="gap-1"
                                disabled={
                                    motivos.current_page >= motivos.last_page
                                }
                                onClick={() => goToPage(motivos.current_page + 1)}
                            >
                                Siguiente
            ...
            >
                <DialogContent className="sm:max-w-lg">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {isEditing
                                ? 'Actualiza la información del motivo.'
                                : 'Registra un nuevo motivo.'}
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <form onSubmit={submit} className="grid gap-4">
                        ...
                        <div className="flex items-center justify-between rounded-lg border p-3">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    {form.data.Estado
                                        ? 'El motivo está activo.'
                                        : 'El motivo está inactivo.'}
                                </p>
                            </div>
                            <Switch
                                id="Estado"
                        ...
                        <DialogFooter>
                            ...
                            <Button type="submit" disabled={form.processing}>
                                ...
                                {isEditing ? 'Guardar cambios' : 'Crear Motivo'}
                            </Button>
                        </DialogFooter>
                    </form>
                </DialogContent>
            ...
            >
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle>Detalle del Motivo</DialogTitle>
                    </DialogHeader>
                    {viewRow && (
